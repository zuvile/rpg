...
class FightMode(GameMode, Cursor):
    ...
    def draw(self, game_state):
        ...
        elif self.is_enemy_turn and not player.in_animation:
    ...
    def play_card(self, player, enemy):
        ...
        if is_key_pressed(KEY_ENTER):
            ...
            self.draw_last_selected_card_effects(player, self.fight_map)
    ...
    def draw_last_selected_card_effects(self, player, game_state):
        ...
        if card.type == CardType.MOVE:
            self.handle_move_card(player, card)
        elif card.type == CardType.HEAL:
            self.handle_heal_card(player, card)

        # self.is_enemy_turn = True

    def handle_heal_card(self, player, card):
        player.hp += card.heal
        self.message_stack.append("You healed for " + str(card.heal) + " hp!")
        player.is_healing = True
        player.heal_animation_start_time = get_time()

    def handle_move_card(self, player, card):
